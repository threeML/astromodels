...
# (e.g. 'CDELT1'), RA and DEC values at reference pixel (e.g. 'CRVAL1')
...
# The code assumes the template is normalized to 1 sr.  If it isn't be default then you
# should set the optional normalization (K) appropriately.  The example template is
# already normalized to 1 sr so we'll keep K set to 1.  Note K is set to 1 and fixed by
# default, we include the following commands as an example of how to manipulate K
shape.K = 1.0
shape.K.fix = True
...
# return the values at various pixels at locations (x,y).  Note the code assumes x=RA
# (degrees) and y=DEC(degrees).  Note the code will return a value of 0 is the pixel is
# outside the template ROI...in this example only the 2nd pixel will have a non-zero
# value
