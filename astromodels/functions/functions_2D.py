...
class Disk_on_sphere(Function2D, metaclass=FunctionMeta):
    ...
    def get_boundaries(self):
        ...
        if (
            ...
            max_lon = self.lon0.value + 2 * max_radius / np.cos(
                max_abs_lat * np.pi / 180.0
            )
...
class Ellipse_on_sphere(Function2D, metaclass=FunctionMeta):
    ...
    def calc_focal_pts(self, lon0, lat0, a, b, theta):
        ...
        lon2, lat2 = vincenty(lon0, lat0, bearing + 180.0, f)
    ...
    def evaluate(self, x, y, lon0, lat0, a, e, theta):
        ...
        self.lon1, self.lat1, self.lon2, self.lat2 = self.calc_focal_pts(
            lon0, lat0, a, b, theta
        )
        self.focal_pts = True
...
class SpatialTemplate_2D(Function2D, metaclass=FunctionMeta):
    ...
    def evaluate(self, x, y, K, hash, ihdu):
        ...
        # X and Y definition are defined by the frame (ICRS,galactic, etc..)
...
class Power_law_on_sphere(Function2D, metaclass=FunctionMeta):
    ...
    def evaluate(self, x, y, lon0, lat0, index, maxr, minr):
        ...
        value = (
            np.less_equal(angsep, maxr)
            * np.power(np.pi / 180.0, index)
            * np.power(
                np.add(
                    np.multiply(angsep, np.greater(angsep, minr)),
                    np.multiply(minr, np.less_equal(angsep, minr)),
                ),
                index,
            )
        )
    ...
    def get_boundaries(self):
        ...
        return (
            ...
            (self.lat0.value - self.maxr.value),
            (self.lat0.value + self.maxr.value),
        )
