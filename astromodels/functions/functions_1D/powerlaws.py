...
class Super_cutoff_powerlaw_fermi(Function1D, metaclass=FunctionMeta):
    r"""
    description :

        PLSuperExpCutoff4 from fermitools, See https://fermi.gsfc.nasa.gov/ssc/data/analysis/scitools/source_models.html#PLSuperExpCutoff4 . Contains both the LogParabola and exponential cutoff PL as special cases. Index2=0 (b=0) corresponds to a LogParabola with alpha=IndexS and beta=d/2. Prefactor = $K$, IndexS = $\gamma$, Scale = $piv$, ExpfactorS = d, Index2 = b.
        
    latex : $ K \left( \frac{E}{piv} \right)^{\gamma - \frac{d}{2} \ln \frac{E}{piv} - \frac{db}{6}\ln^2 \frac{E}{piv} -  \frac{db^2}{24}\ln^3 \frac{E}{piv}  } ~,~~ \mathrm{if} ~ \left| b \ln \frac{E}{piv} \right| < e^{-2} \\
        K \left( \frac{E}{piv} \right)^{\gamma + \frac{d}{b} } \exp \left( \frac{d}{b^2} \left( 1 - \left( \frac{E}{piv}\right)^b \right) \right) ~~ \mathrm{otherwise} $

    parameters :

        K :

            desc : Normalization (differential flux at the pivot value) aka Prefactor
            initial value : 1.0
            transformation : log10
            is_normalization : True
        ...
        IndexS :
            ...
            desc : Photon index at pivot energy aka $\gamma$.
            initial value : -2
            min : -10
            max : 10
        ...
        ExpfactorS :
            ...
            desc : Photon curvature at pivot energy aka d
            initial value : 0.1
            min : 0.0

        Index2 :

            desc : Cutoff strength aka b. Recommended to free this only for very significant sources.
            initial value : 0.6667
            min : 0.0
            max : 10.0
            fix : yes
    ...
    def _set_units(self, x_unit, y_unit):
        # The index is always dimensionless
        self.IndexS.unit = astropy_units.dimensionless_unscaled
        self.ExpfactorS.unit = astropy_units.dimensionless_unscaled
        self.Index2.unit = astropy_units.dimensionless_unscaled
    ...
    # noinspection PyPep8Naming
    def evaluate(self, x, K, piv, IndexS, ExpfactorS, Index2):

        if isinstance(x, astropy_units.Quantity):
            IndexS_ = IndexS.value
            K_ = K.value
            piv_ = piv.value
            Index2_ = Index2.value
            ExpfactorS_ = ExpfactorS.value
            x_ = x.value
            ...
            unit_ = 1.0
            K_, piv_, x_, IndexS_, ExpfactorS_, Index2_ = K, piv, x, IndexS, ExpfactorS, Index2

        result = nb_func.super_cplawf_eval(x_, K_, piv_, IndexS_, ExpfactorS_, Index2_)
