...
def angular_distance_fast(ra1, dec1, ra2, dec2):
    """Compute angular distance using the Haversine formula. For angles in degrees!

    Use this one when you know you will never ask for points at their antipodes.
    If this is not the case, use the angular_distance function which is slower, but
    works also for antipodes.
...
def angular_distance_fast_rad(ra1, dec1, ra2, dec2):
...
def angular_distance(ra1, dec1, ra2, dec2):
    ...
    a set of points and one point. For angles in degrees!
...
def angular_distance_rad(ra1, dec1, ra2, dec2):
    """Returns the angular distance between two points, two sets of points, or
    a set of points and one point. For angles in rad!

    :param ra1: array or float, longitude of first point(s)
    :param dec1: array or float, latitude of first point(s)
    :param ra2: array or float, longitude of second point(s)
    :param dec2: array or float, latitude of second point(s)
    :return: angular distance(s) in rad
    """
...
def spherical_angle_rad(ra0, dec0, ra1, dec1, ra2, dec2):
    ...
    a = angular_distance(ra0, dec0, ra1, dec1)
    b = angular_distance(ra0, dec0, ra2, dec2)
    c = angular_distance(ra2, dec2, ra1, dec1)
    ...
    return np.where(
        denominator == 0,
        np.zeros(len(denominator)),
        np.arccos(numerator / denominator),
    )
